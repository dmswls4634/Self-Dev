...
class LoginForm extends Component {
    submitClick = (e) => {
        ...
        if(this.email_val === '' || this.pwd_val === ''){
            this.sweetalert('이메일과 비밀번호를 확인해주세요.1', '', 'info', '닫기')
        }else{
            ...
            .then( response => {
                ...
                if(userid != null && userid != ''){
                    ...
                    .catch( error => {
                        this.sweetalert('작업중 오류가 발생하였습니다.33', error, 'error', '닫기');
                    });
                    ...
                    this.sweetalert('이메일과 비밀번호를 확인해주세요.2', '', 'info', '닫기')
                }
            })
            .catch( error => {this.sweetalert('이메일과 비밀번호를 확인해주세요.3', '', 'info', '닫기')} );
        }
    }
    ...
    sendEmail = (email, subject, password, e) => {
        ...
        .then( response => {
            if(response.data.success){
                ...
                //this.sweetalert(`작업중 오류가 발생하였습니다.44: ${response.data.message} - ${response.data.error}`, '', 'error', '닫기');
            }
        })
        .catch( error => {
